...
def ppt_figure_png(fig, width=1500, height=820):
    ...
    if value_title:
        if orientation == "h":
            _ = ax.set_xlabel(_clean_plotly_text(value_title), color=muted, fontsize=9)
        else:
            _ = ax.set_ylabel(_clean_plotly_text(value_title), color=muted, fontsize=9)
...
with tab_inventory:
    # =========================================================
    # 4A. MUTI MC TRENDS HEADER & DATA CONTROL
    # =========================================================
    ...
    with trend_action_col:
        ...
        with st.popover("Data Control ▾", use_container_width=True):
            ...
            if saved_workbook_bytes is not None:
                ...
                # The generation now ONLY happens exactly when this button is clicked.
                if st.button(
                    "📊 Export Presentation",
                    use_container_width=True,
                    help="Generate and automatically download the presentation for the selected network scope.",
                ):
                    with st.spinner("Generating presentation..."):
                        ...
                        # Push the download immediately to the browser without using st.download_button
