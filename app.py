...
def get_cloud_storage_config():
    ...
    return {"configured": configured, "url": url, "key": key, "bucket": bucket, "object_name": object_name}
...
def upload_cloud_workbook(file_bytes):
    ...
    if response.status_code not in (200, 201):
        raise RuntimeError(f"Cloud persistence upload failed ({response.status_code}): {response.text[:300]}")
    return True
...
st.markdown(
    ...
    <style>
        ...
        .block-container, [data-testid="stMainBlockContainer"] {
            ...
            padding-left: clamp(0.5rem, 2vw, 1.5rem) !important; padding-right: clamp(0.5rem, 2vw, 1.5rem) !important; margin: 0 !important;
        }
        ...
        div[data-baseweb="tab-panel"] { width: 100% !important; padding-left: 0 !important; padding-right: 0 !important; padding-bottom: 0 !important; padding-top: 1rem !important; }
        div[data-baseweb="tab-panel"] > div, div[data-baseweb="tab-panel"] > div > div[data-testid="stVerticalBlock"] { padding-left: 0 !important; padding-right: 0 !important; }
        [data-testid="stSidebar"], [data-testid="collapsedControl"] { display: none !important; }
        [data-testid="stToolbar"], #MainMenu, header[data-testid="stHeader"], [data-testid="stDecoration"], [data-testid="viewerBadge"] { display: none !important; visibility: hidden !important; }
        header[data-testid="stHeader"] { height: 0 !important; min-height: 0 !important; }

        .data-sync-caption {
            display: block; margin: 0 0 0.28rem 0; color: var(--scm-muted); font-size: 0.62rem;
            font-weight: 850; line-height: 1.2; letter-spacing: 0.09em; text-transform: uppercase; text-align: right; white-space: nowrap;
        }
        .data-sync-shell { width: 100%; min-width: 0; padding-top: 0.02rem; }

        div[data-testid="stButton"] > button {
            min-height: 42px; border-radius: 11px; font-weight: 800; letter-spacing: 0.01em;
            border: 1px solid rgba(99, 102, 241, 0.34); box-shadow: 0 5px 14px rgba(15, 23, 42, 0.06);
        }

        /* Executive Header Button (e.g. Admin Login / Data Control Popover trigger) */
        div[data-testid="stHorizontalBlock"] > div:last-child div[data-testid="stButton"] > button,
        div[data-testid="stHorizontalBlock"] > div:last-child div[data-testid="stPopover"] > button {
            min-height: 44px; border-radius: 12px; border: 1px solid rgba(99, 102, 241, 0.50);
            background: linear-gradient(135deg, rgba(79,70,229,0.98), rgba(37,99,235,0.96));
            color: #ffffff; font-weight: 850; box-shadow: 0 8px 20px rgba(37, 99, 235, 0.18);
            transition: transform 150ms ease, box-shadow 150ms ease, filter 150ms ease;
        }
        div[data-testid="stHorizontalBlock"] > div:last-child div[data-testid="stButton"] > button:hover,
        div[data-testid="stHorizontalBlock"] > div:last-child div[data-testid="stPopover"] > button:hover {
            transform: translateY(-1px); box-shadow: 0 11px 24px rgba(37, 99, 235, 0.24); filter: brightness(1.03);
        }

        div[data-testid="stDialog"] [data-testid="stFileUploader"] section {
            min-height: 118px !important; border-radius: 14px !important;
            border: 1px dashed rgba(99, 102, 241, 0.48) !important; background: rgba(99, 102, 241, 0.035) !important;
        }
        .import-dialog-note {
            padding: 0.72rem 0.85rem; border: 1px solid rgba(148, 163, 184, 0.18); border-radius: 12px;
            background: rgba(148, 163, 184, 0.035); color: var(--scm-muted); font-size: 0.78rem; line-height: 1.5; margin-bottom: 0.85rem;
        }

        /* Responsive Pareto HTML tables */
        .pareto-html-shell { width: 100%; max-width: 100%; min-width: 0; overflow: hidden; border: 1px solid var(--scm-border); border-radius: 12px; }
        .pareto-html-table { width: 100%; max-width: 100%; table-layout: fixed; border-collapse: collapse; font-size: 0.78rem; }
        ...
        .pareto-html-table .center { text-align: center !important; font-variant-numeric: tabular-nums; white-space: nowrap; }
        .pareto-html-table th.center, .pareto-html-table td.center { text-align: center !important; vertical-align: middle !important; }
        .pareto-html-table td.center .pareto-status { margin-left: auto; margin-right: auto; }
        ...
        .hero-grid, .hero-copy { display: block; width: 100%; min-width: 0; max-width: 100%; position: static !important; }
        .hero-kicker, .hero-title, .hero-subtitle { display: block; position: static !important; float: none !important; clear: both; width: 100%; height: auto !important; min-height: 0 !important; padding: 0 !important; transform: none !important; white-space: normal !important; overflow: visible !important; text-overflow: clip !important; overflow-wrap: break-word; word-break: normal; }
        ...
        .metric-card, .metric-card-base { position: relative; box-sizing: border-box; width: 100%; min-width: 0; min-height: 125px; height: 100%; padding: 18px 20px; border: 1px solid var(--scm-border); border-radius: 16px; background: linear-gradient(145deg, rgba(148, 163, 184, 0.02), rgba(148, 163, 184, 0.05)); box-shadow: 0 4px 15px rgba(15, 23, 42, 0.03); overflow: hidden; transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); }
        .metric-card:hover, .metric-card-base:hover { transform: translateY(-4px) scale(1.01); box-shadow: 0 15px 35px rgba(99, 102, 241, 0.12), 0 5px 15px rgba(0, 0, 0, 0.06); border-color: rgba(99, 102, 241, 0.4) !important; }
        .metric-card::before, .metric-card-base::before { content: ""; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; background: var(--scm-indigo); opacity: 0.95; transition: width 0.2s ease; }
        .metric-card:hover::before, .metric-card-base:hover::before { width: 6px; box-shadow: 2px 0 8px rgba(99, 102, 241, 0.5); }
        ...
        .metric-value, .metric-value-sm { font-size: clamp(1.72rem, 1.85vw, 2.05rem); font-weight: 900; line-height: 1.05; margin: 9px 0 6px 0; letter-spacing: -0.025em; }
        ...
        .status-pill strong { color: inherit; font-weight: 850; }
        .status-dot { width: 6px; height: 6px; flex: 0 0 6px; border-radius: 50%; background: var(--scm-green); box-shadow: 0 0 0 3px rgba(16,185,129,0.10); }
        ...
        div[data-baseweb="select"] > div, div[data-testid="stFileUploader"] section { border-radius: 10px; }
        ...
        .pareto-header > span:first-child { min-width: 0; overflow-wrap: anywhere; }
        ...
        .pareto-table-shell { width: 100%; min-width: 0; overflow: hidden; }
        hr { border: none; border-top: 1px solid var(--scm-border); margin: 1.10rem 0; }
        
        .stApp { background-image: radial-gradient(circle at 6% 0%, rgba(99,102,241,0.08), transparent 30%), radial-gradient(circle at 94% 8%, rgba(14,165,233,0.06), transparent 28%); }
        .st-key-scm_executive_header { position: relative; isolation: isolate; background: linear-gradient(135deg, rgba(15,23,42,0.98), rgba(30,41,59,0.96) 55%, rgba(49,46,129,0.94)) !important; border: 1px solid rgba(129,140,248,0.28) !important; border-left: 4px solid #818cf8 !important; box-shadow: 0 18px 46px rgba(15,23,42,0.22) !important; }
        .st-key-scm_executive_header::after { content: ""; position: absolute; width: 220px; height: 220px; right: -70px; top: -120px; border-radius: 50%; background: rgba(99,102,241,0.12); filter: blur(5px); z-index: -1; }
        .st-key-scm_executive_header .hero-title { text-shadow: 0 2px 18px rgba(0,0,0,0.20); }

        .metric-card, .metric-card-base, div[data-testid="stVerticalBlockBorderWrapper"] { backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px); transition: transform 160ms ease, box-shadow 160ms ease, border-color 160ms ease; }
        .metric-card:hover, .metric-card-base:hover, div[data-testid="stVerticalBlockBorderWrapper"]:has(.js-plotly-plot):hover { transform: translateY(-2px); border-color: rgba(99,102,241,0.30) !important; box-shadow: 0 12px 30px rgba(15,23,42,0.10) !important; }
        button[data-baseweb="tab"] { font-weight: 800 !important; letter-spacing: 0.01em !important; }
        button[data-baseweb="tab"][aria-selected="true"] { border-bottom-width: 3px !important; }
        .section-heading .title { font-size: clamp(1.02rem, 1.4vw, 1.20rem); }
        .data-sync-shell + div button, .st-key-open_scm_data_sync_dialog button { border-radius: 11px !important; font-weight: 850 !important; box-shadow: 0 8px 18px rgba(79,70,229,0.18) !important; white-space: nowrap !important; font-size: 0.72rem !important; }
        [data-testid="stFileUploader"] section { transition: border-color 160ms ease, background 160ms ease; }
        [data-testid="stFileUploader"] section:hover { border-color: rgba(99,102,241,0.68) !important; background: rgba(99,102,241,0.055) !important; }

        @media (max-width: 900px) { .trend-heading-inline { min-height: 46px; } .section-heading { grid-template-columns: auto minmax(0, 1fr); } .section-heading .subtitle { grid-column: 2; text-align: left; } }
    </style><br>
    """,
...
st.markdown(
    f"""
    <style>
        :root {{ --scm-page-bg: {theme_tokens["page_bg"]}; --scm-surface: {theme_tokens["surface"]}; --scm-surface-2: {theme_tokens["surface_2"]}; --scm-card-bg: {theme_tokens["card_bg"]}; --scm-input-bg: {theme_tokens["input_bg"]}; --scm-text: {theme_tokens["text"]}; --scm-muted: {theme_tokens["muted"]}; --scm-border: {theme_tokens["border"]}; --scm-soft-border: {theme_tokens["soft_border"]}; --scm-table-head: {theme_tokens["table_head"]}; --scm-hover: {theme_tokens["hover"]}; --scm-hero-bg: {theme_tokens["hero_bg"]}; --scm-hero-title: {theme_tokens["hero_title"]}; --scm-hero-subtitle: {theme_tokens["hero_subtitle"]}; --scm-hero-kicker: {theme_tokens["hero_kicker"]}; --scm-theme-shadow: {theme_tokens["shadow"]}; }}
        ...
        [data-testid="stMainBlockContainer"] {{ color: var(--scm-text) !important; }}
        [data-testid="stMarkdownContainer"], [data-testid="stMarkdownContainer"] p, [data-testid="stMarkdownContainer"] li, [data-testid="stMarkdownContainer"] h1, [data-testid="stMarkdownContainer"] h2, [data-testid="stMarkdownContainer"] h3, [data-testid="stMarkdownContainer"] h4, [data-testid="stWidgetLabel"], [data-testid="stWidgetLabel"] p {{ color: var(--scm-text); }}
        [data-testid="stCaptionContainer"], [data-testid="stCaptionContainer"] p {{ color: var(--scm-muted) !important; }}
        ...
        .hero-title {{ color: var(--scm-hero-title) !important; }}
        .hero-subtitle {{ color: var(--scm-hero-subtitle) !important; }}
        .hero-kicker {{ color: var(--scm-hero-kicker) !important; }}
        ...
        [data-baseweb="popover"] [role="option"] {{ color: var(--scm-text) !important; }}
        [data-baseweb="popover"] [role="option"]:hover {{ background: var(--scm-hover) !important; }}
        button[data-baseweb="tab"] {{ color: var(--scm-muted) !important; }}
        button[data-baseweb="tab"][aria-selected="true"] {{ color: var(--scm-text) !important; }}
        ...
        .status-pill, .info-chip, .pareto-count {{ border-color: var(--scm-border) !important; }}
        hr {{ border-top-color: var(--scm-border) !important; }}
        [data-testid="stDialog"] [role="dialog"] {{ background: var(--scm-surface) !important; color: var(--scm-text) !important; }}
        div[data-testid="stDialog"] [data-testid="stFileUploader"] section {{ background: var(--scm-surface-2) !important; }}
        .import-dialog-note {{ background: var(--scm-surface-2) !important; border-color: var(--scm-border) !important; color: var(--scm-muted) !important; }}

        .st-key-scm_executive_header {{ box-sizing: border-box; width: 100%; min-width: 0; margin: 0.08rem 0 0.92rem 0; padding: clamp(20px, 1.65vw, 28px) clamp(22px, 2.15vw, 34px); border: 1px solid var(--scm-border); border-left: 4px solid var(--scm-indigo); border-radius: 18px; background: var(--scm-hero-bg); box-shadow: var(--scm-theme-shadow); overflow: visible; }}
...
def process_excel_file(file_path_or_buffer):
    ...
    try:
        ...
        raise ValueError(f"Unable to open the Excel workbook: {exc}") from exc
    ...
    def parse_kpi_sheet(sheet_name):
        ...
        empty_kpi_df = pd.DataFrame(columns=["period", "after_po", "per_branch", "class_a_out", "before_po", "overall_doi", "class_a_doi"])
        if df_raw.empty or df_raw.shape[1] < 2: return empty_kpi_df
        ...
        def parse_header_date(value):
            ...
            if isinstance(value, (pd.Timestamp, datetime, date, np.datetime64)):
                ...
                if not value_text: return pd.NaT
                parsed = pd.to_datetime(value_text, errors="coerce")
            if pd.isna(parsed): return pd.NaT
            parsed = pd.Timestamp(parsed)
            if parsed.year < 2000 or parsed.year > 2100: return pd.NaT
        ...
        if best_date_row is None or not best_date_columns:
            raise ValueError(f"No valid KPI date headers found in sheet '{sheet_name}'.")
        ...
        for kpi_name, col_name in kpis_to_extract.items():
            idx = df_raw.index[first_col_as_text.str.contains(kpi_name, regex=False, na=False)].tolist()
            if idx:
        ...
        metric_cols = ["after_po", "per_branch", "class_a_out", "before_po", "overall_doi", "class_a_doi"]
        ...
        clean_df = clean_df.sort_values("period").groupby("period", as_index=False, sort=True).agg({col: last_valid for col in metric_cols})
        ...
        clean_df = clean_df.drop_duplicates(subset=["period"], keep="last").sort_values("period").reset_index(drop=True)
        return clean_df
...
# =========================================================
# 4. ADMIN LOGIN & DATA SYNCHRONIZATION 
# =========================================================
cloud_config = get_cloud_storage_config()
saved_workbook_bytes, storage_source = initialize_persistent_workbook()

@st.dialog("Admin Authentication", width="small")
...
def data_sync_dialog():
    ...
    with action_col:
        do_import = st.button("Validate & Sync", type="primary", use_container_width=True, key="scm_data_sync_confirm_button")
    with info_col:
        st.caption("Import and synchronize the SCM Excel workbook.")
...
# =========================================================
# 5. HELPERS (WITH CORRECTED PLOTLY FILLCOLOR & ANIMATIONS)
# =========================================================
...
def stock_status_style_class(status_value):
    ...
    if normalized in {"stockout", "stock out", "out of stock", "oos"}: return "stockout"
    if "critical" in normalized: return "critical"
    if normalized in {"low", "low stock", "below min", "below minimum"}: return "low"
    if normalized in {"ok", "normal", "healthy", "available", "in stock", "instock"}: return "ok"
    if normalized in {"overstock", "over stock", "excess", "excess stock"}: return "overstock"
    return "neutral"
...
def render_stock_status_legend(df):
    ...
    for value in df["stock_status"].fillna("").astype(str):
        label, key = value.strip(), value.strip().casefold()
        if not label or key in seen: continue
        seen.add(key)
        observed.append(label)
    if not observed: return
    chips = [f"<span class='pareto-status {stock_status_style_class(lbl)}'>{html.escape(lbl)}</span>" for lbl in observed]
    st.markdown("<div class='stock-status-legend'><span class='stock-status-legend-label'>Stock Status</span>" + "".join(chips) + "</div>", unsafe_allow_html=True)
...
def apply_executive_bar_style(fig, *, accent_color, value_axis_title, value_max, category_order, orientation="v", percent=True, height=425, right_margin=28):
    _ = fig.update_traces(
        marker=dict(color=accent_color, line=dict(color="rgba(255, 255, 255, 0.15)", width=1.5)),
        opacity=0.92,
        textfont=dict(size=13, color=PLOTLY_CHART_TEXT, family="Arial", weight="bold"),
        cliponaxis=False,
    )

    value_axis = dict(title=dict(text=value_axis_title, font=dict(size=12, color=PLOTLY_CHART_MUTED)), range=[0, value_max], gridcolor=PLOTLY_CHART_GRID, zeroline=False, showline=False, tickfont=dict(size=11, color=PLOTLY_CHART_MUTED), automargin=True)
    if percent: value_axis["ticksuffix"] = "%"
    category_axis = dict(title="", type="category", categoryorder="array", categoryarray=category_order, showgrid=False, showline=True, linecolor=PLOTLY_CHART_AXIS, tickfont=dict(size=11, color=PLOTLY_CHART_MUTED), automargin=True)
    ...
    _ = fig.update_layout(
        ...
        height=height, margin=dict(t=76, b=52, l=42 if orientation == "v" else 20, r=right_margin),
        showlegend=False, bargap=0.25 if orientation == "v" else 0.22, hovermode="closest", dragmode=False,
        font=dict(color=PLOTLY_CHART_TEXT),
        title=dict(x=0.02, xanchor="left", y=0.97, yanchor="top", font=dict(size=17, color=PLOTLY_CHART_TEXT)),
        hoverlabel=dict(bgcolor=PLOTLY_HOVER_BG, bordercolor=accent_color, font=dict(color=PLOTLY_HOVER_TEXT, size=12)),
        uniformtext=dict(minsize=10, mode="hide"), xaxis=xaxis, yaxis=yaxis,
...
def prepare_chart_series(df, y_col):
    ...
    chart_df = chart_df.dropna(subset=["period", y_col]).sort_values("period").drop_duplicates(subset=["period"], keep="last")
...
def create_styled_line_chart(df, y_col, title, subtitle, line_color, is_weekly, is_percentage=True, fill=False):
    ...
    if chart_df.empty:
        _ = fig.add_annotation(text="No valid data available for this KPI", x=0.5, y=0.5, xref="paper", yref="paper", showarrow=False, font=dict(size=14, color="#94a3b8"))
        _ = fig.update_layout(template=PLOTLY_TEMPLATE, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", height=350, margin=dict(t=72, b=34, l=42, r=20), title=dict(text=f"{title}<br><span style='font-size:10px'>{subtitle}</span>", x=0.02, xanchor="left"), xaxis=dict(visible=False), yaxis=dict(visible=False))
        return fig
    ...
    _ = fig.add_trace(go.Scatter(x=chart_x, y=plot_y, mode="lines", line=dict(shape=line_shape, width=8, color=line_color), opacity=0.15, hoverinfo="skip", showlegend=False))

    # Main Trace - WITH STRICT `None` FALLBACK FIX
    ...
    _ = fig.add_trace(
        go.Scatter(
            ...
            text=text_labels, textposition=text_positions, textfont=dict(size=11, family="Arial", color=PLOTLY_CHART_TEXT),
            line=dict(shape=line_shape, width=3.5, color=line_color), marker=dict(size=9, color=line_color, line=dict(width=2, color=PLOTLY_HOVER_BG)),
    ...
    if len(chart_df) >= 2:
        ...
        if valid_trend.sum() >= 2 and np.ptp(trend_x[valid_trend]) > 0:
            slope, intercept = np.polyfit(trend_x[valid_trend], trend_source_y[valid_trend], 1)
            fitted_y = slope * trend_x + intercept

            actual_valid = trend_source_y[np.isfinite(trend_source_y)]
            actual_max, actual_min = float(np.nanmax(actual_valid)), float(np.nanmin(actual_valid))
            ...
            normalized_fit = (fitted_y - np.nanmin(fitted_y[valid_trend])) / fitted_range if fitted_range > 0 else np.full_like(fitted_y, 0.5, dtype=float)
            ...
            if fitted_delta > flat_threshold: trend_direction = "Upward"
            elif fitted_delta < -flat_threshold: trend_direction = "Downward"
            else: trend_direction = "Stable"

            direction_symbol = {"Upward": "↑", "Downward": "↓", "Stable": "→"}[trend_direction]

            _ = fig.add_trace(go.Scatter(x=chart_x, y=trend_y, name=f"Trend Direction {direction_symbol}", mode="lines", line=dict(width=2.4, dash="dash", color="rgba(100,116,139,0.95)"), hovertemplate=(f"Trend direction: <b>{trend_direction} {direction_symbol}</b><br><span style='font-size:10px'>Guide positioned above Actual for readability</span><extra></extra>"), connectgaps=False))

    max_observed = pd.to_numeric(plot_y, errors="coerce").max()
    if trend_y is not None and len(trend_y):
        trend_max = np.nanmax(trend_y)
        if np.isfinite(trend_max): max_observed = max(max_observed, trend_max)
    ...
    xaxis_config = dict(
        ...
        tickangle=0, linecolor="rgba(148,163,184,0.18)", tickmode="array", tickvals=chart_x.tolist(), ticktext=tick_text.tolist() if is_weekly else tick_text,
        showspikes=True, spikecolor=PLOTLY_CHART_MUTED, spikethickness=1, spikedash="dot", spikemode="across"
    )

    _ = fig.update_layout(
        template=PLOTLY_TEMPLATE, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", height=365, margin=dict(t=82, b=44, l=46, r=20),
        hovermode="x unified", hoverlabel=dict(bgcolor=PLOTLY_HOVER_BG, bordercolor=PLOTLY_HOVER_BORDER, font=dict(color=PLOTLY_HOVER_TEXT, size=12)),
        showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.015, xanchor="right", x=0.99, bgcolor="rgba(0,0,0,0)", font=dict(size=10, color="#94a3b8")),
        title=dict(text=f"{title}<br><span style='font-size:10px; color:#94a3b8;'>{subtitle} • {len(chart_df)} DATA PERIOD(S) • LATEST {latest_text.upper()}</span>", x=0.02, y=0.96, xanchor="left", yanchor="top", font=dict(size=17, family="Arial", color=PLOTLY_CHART_TEXT)),
        xaxis=xaxis_config, yaxis=dict(showgrid=True, gridwidth=1, gridcolor="rgba(148,163,184,0.1)", zeroline=False, tickformat=tick_format, range=[0, y_max]),
        transition=dict(duration=500, easing="cubic-in-out")
    )
...
def performance_card(title, value, note):
    return f"<div class='metric-card'><div class='metric-title'>{title}</div><div class='metric-value-sm'>{value}%</div><div class='metric-footnote'>{note}</div></div>"

def card_html(title, value, badge_text, color_theme, icon):
    return f"<div class='metric-card-base'><div class='metric-header'>{title}<div class='icon-box'>{icon}</div></div><div class='metric-value'>{value}%</div><div><span class='badge badge-{color_theme}'>{badge_text}</span></div></div>"
...
def ppt_theme_colors():
    if SCM_IS_DARK:
        return {"bg": "070B14", "surface": "0F172A", "surface2": "111827", "text": "F8FAFC", "muted": "94A3B8", "border": "334155", "accent": "6366F1", "red": "F43F5E", "amber": "F59E0B", "green": "10B981", "blue": "0EA5E9"}
    return {"bg": "F6F8FC", "surface": "FFFFFF", "surface2": "F8FAFC", "text": "0F172A", "muted": "64748B", "border": "CBD5E1", "accent": "4F46E5", "red": "F43F5E", "amber": "F59E0B", "green": "10B981", "blue": "0EA5E9"}
...
def ppt_add_title(slide, title, subtitle="", colors=None):
    ...
    shape = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(0), Inches(0), PPT_W, Inches(0.14))
    _ = shape.fill.solid()
...
def _mpl_hex(value, fallback="#94a3b8"):
    ...
    if text.lower() == "transparent": return fallback
    return text
...
def ppt_figure_png(fig, width=1500, height=820):
    ...
    orientation = getattr(traces[0], "orientation", None) or "v" if traces else "v"
    ...
    for trace in traces:
        ...
        if trace_type == "scatter":
            ...
            if fill_value in {"tozeroy", "tonexty"}:
                _ = ax.fill_between(x_pos, [0] * len(y_num), y_num, color=color, alpha=0.12, zorder=2)
            ...
            if ori == "h":
                ...
                if text_values:
                    for i, value in enumerate(values):
                        ...
                        _ = ax.text(value, i, f"  {label}", va="center", ha="left", fontsize=9, fontweight="bold", color=text_color, clip_on=False)
            else:
                ...
                if text_values:
                    for i, value in enumerate(values):
                        label = text_values[i] if i < len(text_values) else value
                        _ = ax.text(i, value, f"{label}", va="bottom", ha="center", fontsize=9, fontweight="bold", color=text_color, clip_on=False)
    ...
    if title_text: _ = ax.set_title(title_text, loc="left", color=text_color, fontsize=14, fontweight="bold", pad=18)
    ...
    value_axis, category_axis = (xaxis, yaxis) if orientation == "h" else (yaxis, xaxis)

    value_title = getattr(getattr(value_axis, "title", None), "text", "") if value_axis is not None else ""
    if value_title:
        if orientation == "h": _ = ax.set_xlabel(_clean_plotly_text(value_title), color=muted, fontsize=9)
        else: _ = ax.set_ylabel(_clean_plotly_text(value_title), color=muted, fontsize=9)

    if orientation != "h" and category_values is not None:
        _ = ax.set_xticks(range(len(category_values)))
        _ = ax.set_xticklabels(category_values)
        _ = ax.tick_params(axis="x", labelrotation=0)
    ...
    if tickformat in {".0%", "0%", ".1%"}:
        ...
        if orientation != "h": _ = ax.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=100, decimals=0))
        else: _ = ax.xaxis.set_major_formatter(mticker.PercentFormatter(xmax=100, decimals=0))

    handles, labels = ax.get_legend_handles_labels()
    if handles and any(labels): _ = ax.legend(loc="upper right", frameon=False, fontsize=8, labelcolor=muted)

    _ = mpl_fig.subplots_adjust(left=0.10, right=0.97, top=0.85, bottom=0.17)
    output = io.BytesIO()
    _ = mpl_fig.savefig(output, format="png", dpi=dpi, facecolor=panel, edgecolor=panel, bbox_inches="tight", pad_inches=0.12)
    _ = plt.close(mpl_fig)
...
def ppt_add_table(slide, df, x, y, w, h, title, colors=None, font_size=7.4):
    ...
    if df is None or df.empty:
        ...
        empty.text_frame.paragraphs[0].text = "No model records available."
        empty.text_frame.paragraphs[0].font.size = Pt(9)
        empty.text_frame.paragraphs[0].font.color.rgb = ppt_rgb(colors["muted"])
        return
    ...
    if cols == 7:
        total = sum(widths)
        for i, frac in enumerate(widths): table.columns[i].width = int(w * (frac / total))
    else:
        for i in range(cols): table.columns[i].width = int(w / cols)
    ...
    for r, (_, row) in enumerate(df.iterrows(), start=1):
        for c, header in enumerate(headers):
            ...
            if header == "Status":
                ...
                status_color = {"stockout": colors["red"], "critical": "F97316", "low": colors["amber"], "ok": colors["green"], "overstock": colors["blue"]}.get(status_class, colors["muted"])
                p.font.color.rgb = ppt_rgb(status_color)
                p.font.bold = True

def exclude_greatwall_for_presentation(raw_data):
    if raw_data is None or raw_data.empty: return raw_data.copy() if hasattr(raw_data, "copy") else raw_data
    filtered = raw_data.copy()
    if "model" not in filtered.columns: return filtered
    model_text = filtered["model"].fillna("").astype(str).str.strip().str.casefold().str.replace(r"[^a-z0-9]+", "", regex=True)
    return filtered.loc[~(model_text.eq("greatwall") | model_text.str.contains("greatwall", na=False))].copy()
...
def build_scm_presentation(raw_data, kpi_ytd, kpi_weekly, selected_area):
    ...
    if selected_area != "All Areas": selected_area_data = selected_area_data[selected_area_data["area"] == selected_area].copy()
    ...
    _ = scope_box.fill.solid(); scope_box.fill.fore_color.rgb = ppt_rgb(colors["surface2"]); scope_box.line.color.rgb = ppt_rgb(colors["border"])
    st_box = slide.shapes.add_textbox(Inches(1.33), Inches(3.27), Inches(3.7), Inches(0.9))
    stp = st_box.text_frame.paragraphs[0]
    stp.text = f"NETWORK SCOPE\n{selected_area}"
    stp.font.name = "Aptos"; stp.font.size = Pt(13); stp.font.bold = True; stp.font.color.rgb = ppt_rgb(colors["text"])

    stats = slide.shapes.add_textbox(Inches(5.75), Inches(3.08), Inches(6.0), Inches(1.45))
    tf = stats.text_frame
    _ = tf.clear()
    for idx, (label, val) in enumerate([("Branches", branch_count), ("Models", model_count), ("Stock records", f"{record_count:,}")]):
        p = tf.paragraphs[0] if idx == 0 else tf.add_paragraph()
        p.text = f"{label}: {val}"; p.font.name = "Aptos"; p.font.size = Pt(12); p.font.color.rgb = ppt_rgb(colors["text"]); p.space_after = Pt(6)

    latest_dates = pd.concat([pd.to_datetime(kpi_ytd.get("period", pd.Series(dtype="datetime64[ns]")), errors="coerce"), pd.to_datetime(kpi_weekly.get("period", pd.Series(dtype="datetime64[ns]")), errors="coerce")], ignore_index=True).dropna()
    ...
    # Trend slides
    trend_specs = [("class_a_doi", "MC Class A DoI", "CLASS A DAYS OF INVENTORY", "#7c3aed", False, True), ("overall_doi", "Days of Inventory", "OVERALL INVENTORY COVERAGE", "#2563eb", False, True), ("per_branch", "Per Branch OOS", "STOCKOUT RATE", "#0ea5e9", True, False), ("class_a_out", "Overall Class A Rate", "CLASS A STOCKOUT RATE", "#f43f5e", True, False), ("before_po", "Overall Before PO Balance", "STOCKOUT RATE BEFORE PO BALANCE", "#f59e0b", True, False), ("after_po", "Overall After PO Balance", "STOCKOUT RATE AFTER PO BALANCE", "#10b981", True, True)]
    for label, source in [("YTD", kpi_ytd), ("Weekly", kpi_weekly)]:
        figs = [create_styled_line_chart(source, key, title, subtitle, color, is_weekly=(label == "Weekly"), is_percentage=pct, fill=fill) for key, title, subtitle, color, pct, fill in trend_specs]
        for start in range(0, len(figs), 2):
            slide = prs.slides.add_slide(blank)
            _ = ppt_add_background(slide, colors)
            _ = ppt_add_title(slide, f"MUTI MC Trends — {label}", f"Same KPI presentation as dashboard • {selected_area} • Source dates only", colors)
    ...
    for area, a_df in selected_area_data.groupby("area", sort=True, dropna=True):
        ...
        avg_area_rate = round_half_up((calculate_stockout_rate(a_df, "Class A") + calculate_stockout_rate(a_df, "Class B") + calculate_stockout_rate(a_df, "Class C")) / 3)
        ...
        area_rates.append({"Area": area, "Average Stock Out Rate": avg_area_rate, "Class A Stock Out Rate": class_a_rate, "Class A Stock Out Count": stockout_count, "Class A Total Stock Status Count": total_count})
    ...
    cards = [("Class A Rate", f"{rate_a}%", "HIGH PRIORITY RISK", colors["red"]), ("Class B Rate", f"{rate_b}%", "MEDIUM PRIORITY RISK", colors["amber"]), ("Class C Rate", f"{rate_c}%", "LOW PRIORITY RISK", colors["green"]), ("Average Rate", f"{avg_rate}%", "PERFORMANCE INDEX", colors["blue"])]
    ...
    detail_box.fill.fore_color.rgb = ppt_rgb(colors["surface"]); detail_box.line.color.rgb = ppt_rgb(colors["border"])
    detail = slide.shapes.add_textbox(Inches(0.82), Inches(3.95), Inches(11.6), Inches(1.8))
    tf = detail.text_frame
    _ = tf.clear()
    bullets = [f"Network scope: {selected_area}", f"Active branches: {branch_count:,}", f"Active models: {model_count:,}", f"Stock status records: {record_count:,}", "Rates are derived from the active Raw_Data records using the dashboard's existing Class A/B/C rate logic."]
    ...
    if not ca.empty:
        ...
        branch_rank_summary = ca.groupby(["area", "branch"], as_index=False).agg(stockout=("_is_stockout", "sum"), total=("_is_stockout", "size"))
    ...
    for branch in branches:
        ...
        branch_cards = [("Branch Class A Rate", f"{a}%", "HIGH PRIORITY RISK", colors["red"]), ("Branch Class B Rate", f"{b}%", "MEDIUM PRIORITY RISK", colors["amber"]), ("Branch Class C Rate", f"{c}%", "LOW PRIORITY RISK", colors["green"]), ("Branch Average", f"{avg}%", "PERFORMANCE INDEX", colors["blue"])]
        ...
        for idx, line in enumerate(lines):
            p = tf.paragraphs[0] if idx == 0 else tf.add_paragraph()
            p.text = "• " + line
            p.font.name = "Aptos"; p.font.size = Pt(10.5); p.font.color.rgb = ppt_rgb(colors["text"]); p.space_after = Pt(5)
...
def build_branch_request_excel(report_df, requesting_branch=None):
    ...
    if "Branch" in export_df.columns:
        export_df = export_df[export_df["Branch"].fillna("").astype(str).str.strip() == branch_name].copy()
    if export_df.empty:
        raise ValueError(f"No request lines are available for branch '{branch_name}'.")
    ...
    branch_area = ""
    if "Area" in export_df.columns:
        area_values = [v for v in export_df["Area"].fillna("").astype(str).str.strip().unique().tolist() if v]
        branch_area = area_values[0] if area_values else ""

    sheet_name = "Branch Request Report"
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        wb = writer.book
        ws = wb.create_sheet(title=sheet_name)
        # Remove the default sheet that ExcelWriter may create when nothing is written via pandas.
        for default_name in list(wb.sheetnames):
            if default_name != sheet_name and default_name.lower().startswith("sheet"):
                del wb[default_name]

        # Portrait-friendly 6-column canvas.
        for col, width in {"A": 3.2, "B": 21, "C": 18, "D": 3.2, "E": 21, "F": 20}.items():
        ...
        ws["A2"] = "MUTI MC SCM Executive Control Tower • Inventory Request Validation"
        ws["A2"].font = Font(bold=True, size=9, color="CBD5E1")
        ws["A2"].fill = PatternFill("solid", fgColor=navy)
        ws["A2"].alignment = Alignment(horizontal="left", vertical="center")
        ws.row_dimensions[2].height = 20

        # Requesting branch gets top visual priority.
        ws.merge_cells("A4:F4")
        ws["A4"] = "REQUESTING BRANCH"
        ws["A4"].font = Font(bold=True, size=9, color=slate)
        ws["A4"].fill = PatternFill("solid", fgColor=label_fill)
        ws["A4"].alignment = Alignment(horizontal="center", vertical="center")
        ws["A4"].border = box_border

        ws.merge_cells("A5:F6")
        ws["A5"] = branch_name
        ws["A5"].font = Font(bold=True, size=19, color=indigo)
        ws["A5"].fill = PatternFill("solid", fgColor=white)
        ...
        for left_label, left_value, right_label, right_value in metadata:
            ws[ f"A{meta_row}" ] = left_label
            ws.merge_cells(start_row=meta_row, start_column=2, end_row=meta_row, end_column=3)
            ws[ f"B{meta_row}" ] = left_value
            ws[ f"D{meta_row}" ] = right_label
            ws.merge_cells(start_row=meta_row, start_column=5, end_row=meta_row, end_column=6)
            ws[ f"E{meta_row}" ] = right_value
            for coord in [f"A{meta_row}", f"D{meta_row}"]:
                ws[coord].font = Font(bold=True, size=8, color=slate)
                ws[coord].fill = PatternFill("solid", fgColor=label_fill)
                ws[coord].alignment = Alignment(horizontal="left", vertical="center")
            for coord in [f"B{meta_row}", f"E{meta_row}"]:
                ws[coord].font = Font(bold=True, size=10, color=text_dark)
                ws[coord].fill = PatternFill("solid", fgColor=white)
                ws[coord].alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
            for c in range(1, 7):
                ws.cell(meta_row, c).border = box_border
        ...
        for item_no, (_, item) in enumerate(export_df.reset_index(drop=True).iterrows(), start=1):
            ...
            status_fill, status_font = status_colors.get(status_class, ("F1F5F9", slate))
            request_check = str(item.get("Request Check", "") or "").strip()
            is_valid = request_check.startswith("VALIDATED")

            # Model header: separate block per requested model, rather than one wide table row.
            ws.merge_cells(start_row=row_cursor, start_column=1, end_row=row_cursor, end_column=6)
            header = ws.cell(row_cursor, 1)
            header.value = f"REQUEST {item_no:02d}  •  {model}"
            header.font = Font(bold=True, size=11, color=white)
            header.fill = PatternFill("solid", fgColor=blue)
            header.alignment = Alignment(horizontal="left", vertical="center")
            for c in range(1, 7):
                ws.cell(row_cursor, c).border = box_border
            ws.row_dimensions[row_cursor].height = 24
            row_cursor += 1

            detail_rows = [
                ("REQUEST QUANTITY", item.get("Request Quantity", 0), "CURRENT INVENTORY", item.get("Inventory", "")),
                ("STOCK STATUS", status, "CURRENT DOI", item.get("Current DoI", "")),
                ("NEW INVENTORY", item.get("New Inventory", ""), "NEW DOI", item.get("New DoI", "")),
                ("CLASS", item.get("Class", ""), "SUGGESTED TRANSFER", item.get("Suggested Transfer", "")),
                ("REQUEST CHECK", request_check, "IMPLIED AVG DAILY SALES", item.get("Implied Avg Daily Sales", "")),
            ]

            for left_label, left_value, right_label, right_value in detail_rows:
                ws.cell(row_cursor, 1).value = left_label
                ws.merge_cells(start_row=row_cursor, start_column=2, end_row=row_cursor, end_column=3)
                ws.cell(row_cursor, 2).value = left_value
                ws.cell(row_cursor, 4).value = right_label
                ws.merge_cells(start_row=row_cursor, start_column=5, end_row=row_cursor, end_column=6)
                ws.cell(row_cursor, 5).value = right_value

                for c in (1, 4):
                    cell = ws.cell(row_cursor, c)
                    cell.font = Font(bold=True, size=8, color=slate)
                    cell.fill = PatternFill("solid", fgColor=label_fill)
                    cell.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
                for c in (2, 5):
                    cell = ws.cell(row_cursor, c)
                    cell.font = Font(bold=True, size=9, color=text_dark)
                    cell.fill = PatternFill("solid", fgColor=white)
                    cell.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)

                # Status and request-check highlighting.
                if left_label == "STOCK STATUS":
                    ws.cell(row_cursor, 2).fill = PatternFill("solid", fgColor=status_fill)
                    ws.cell(row_cursor, 2).font = Font(bold=True, size=9, color=status_font)
                if left_label == "REQUEST CHECK":
                    ws.cell(row_cursor, 2).fill = PatternFill("solid", fgColor="DCFCE7" if is_valid else "FEE2E2")
                    ws.cell(row_cursor, 2).font = Font(bold=True, size=9, color="166534" if is_valid else "B91C1C")

                # DoI formatting.
                if right_label in {"CURRENT DOI", "NEW DOI", "IMPLIED AVG DAILY SALES"}:
                    ws.cell(row_cursor, 5).number_format = "0.00"
            ...
            # Remarks / justification spans the page width and wraps.
            ws.merge_cells(start_row=row_cursor, start_column=1, end_row=row_cursor, end_column=6)
            remarks_label = ws.cell(row_cursor, 1)
            remarks_label.value = "REMARKS / JUSTIFICATION"
            remarks_label.font = Font(bold=True, size=8, color="9A3412")
            remarks_label.fill = PatternFill("solid", fgColor="FFEDD5")
            remarks_label.alignment = Alignment(horizontal="left", vertical="center")
            for c in range(1, 7):
                ws.cell(row_cursor, c).border = box_border
            ws.row_dimensions[row_cursor].height = 20
            row_cursor += 1

            ws.merge_cells(start_row=row_cursor, start_column=1, end_row=row_cursor + 1, end_column=6)
            remarks_cell = ws.cell(row_cursor, 1)
            remarks_cell.value = str(item.get("Remarks / Justification", "") or "").strip() or "—"
            remarks_cell.font = Font(size=9, color="9A3412")
            remarks_cell.fill = PatternFill("solid", fgColor=remarks_fill)
            remarks_cell.alignment = Alignment(horizontal="left", vertical="top", wrap_text=True)
            for rr in range(row_cursor, row_cursor + 2):
                for c in range(1, 7):
                    ws.cell(rr, c).border = box_border
            ws.row_dimensions[row_cursor].height = 25
            ws.row_dimensions[row_cursor + 1].height = 25
            row_cursor += 2

            # Calculation note as a compact footnote inside each request block.
            ws.merge_cells(start_row=row_cursor, start_column=1, end_row=row_cursor, end_column=6)
            note_cell = ws.cell(row_cursor, 1)
            note_cell.value = f"Projection note: {str(item.get('Calculation Note', '') or '').strip()}"
            note_cell.font = Font(size=8, italic=True, color=slate)
            note_cell.fill = PatternFill("solid", fgColor=soft_indigo)
        ...
        ws.merge_cells(start_row=row_cursor, start_column=1, end_row=row_cursor, end_column=6)
        ws.cell(row_cursor, 1).value = (
            "Projection basis: Implied Average Daily Sales = Current Inventory ÷ Current DoI. "
            "New Inventory = Current Inventory + Request Quantity. New DoI = New Inventory ÷ Implied Average Daily Sales."
        )
        ws.cell(row_cursor, 1).font = Font(size=8, italic=True, color=slate)
        ws.cell(row_cursor, 1).fill = PatternFill("solid", fgColor=light_slate)
        ws.cell(row_cursor, 1).alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
        for c in range(1, 7):
            ws.cell(row_cursor, c).border = box_border
        ws.row_dimensions[row_cursor].height = 38
        row_cursor += 3

        signoff_specs = [
            (1, 2, "Prepared / Reviewed By"),
            (3, 4, "Approved By"),
            (5, 6, "Date"),
        ]
        for start_col, end_col, label in signoff_specs:
            ws.merge_cells(start_row=row_cursor, start_column=start_col, end_row=row_cursor, end_column=end_col)
            ws.cell(row_cursor, start_col).value = "________________________"
            ws.cell(row_cursor, start_col).alignment = Alignment(horizontal="center")
            ws.cell(row_cursor, start_col).font = Font(size=9, color=slate)
            ws.merge_cells(start_row=row_cursor + 1, start_column=start_col, end_row=row_cursor + 1, end_column=end_col)
            ws.cell(row_cursor + 1, start_col).value = label
        ...
        ws.print_area = f"A1:F{row_cursor + 1}"
        ws.oddFooter.left.text = branch_name
...
with tab_inventory:
        ...
        if not st.session_state.get("is_admin_logged_in", False):
            ...
            with st.popover("Data Control ▾", use_container_width=True):
                ...
                if saved_workbook_bytes is not None:
                    ...
                    if st.button("📊 Export Presentation", use_container_width=True, help="Generate and automatically download the presentation for the selected network scope."):
                        with st.spinner("Generating presentation..."):
                            presentation_bytes = cached_scm_presentation(saved_workbook_bytes, export_area, SCM_THEME)
                            ...
                            dl_link = f'''<a id="auto-download" href="data:application/vnd.openxmlformats-officedocument.presentationml.presentation;base64,{b64}" download="{presentation_name}"></a><script>document.getElementById('auto-download').click();</script>'''
                            _ = components.html(dl_link, height=0, width=0)

    import_success = st.session_state.pop("scm_import_success", None)
    if import_success: st.success(import_success)
    ...
    if saved_workbook_bytes is None:
        st.info("Log in using 'Admin Login' to upload your workbook.")
        st.stop()
    ...
    try:
        ...
        st.info("Use a genuine Microsoft Excel .xlsx or .xls workbook.")
        st.stop()

    all_kpi_dates = pd.concat([kpi_ytd["period"] if "period" in kpi_ytd else pd.Series(dtype="datetime64[ns]"), kpi_weekly["period"] if "period" in kpi_weekly else pd.Series(dtype="datetime64[ns]")], ignore_index=True)
    all_kpi_dates = pd.to_datetime(all_kpi_dates, errors="coerce").dropna()
    dashboard_latest_date = all_kpi_dates.max().strftime("%d %b %Y") if not all_kpi_dates.empty else "No KPI date"
    persistence_label = "Cloud persistent" if storage_source == "Cloud • Supabase" else "Local fallback"
    ...
    with control_col1:
        timeframe = st.selectbox("TIMEFRAME", ["Year-to-Date (YTD)", "Weekly View"], index=0)
    ...
    with row1_left:
        with st.container(border=True):
            _ = st.plotly_chart(create_styled_line_chart(kpi_data, "class_a_doi", "MC Class A DoI", "CLASS A DAYS OF INVENTORY", "#7c3aed", is_weekly=is_weekly, is_percentage=False, fill=True), width="stretch")

    with row1_right:
        with st.container(border=True):
            _ = st.plotly_chart(create_styled_line_chart(kpi_data, "overall_doi", "Days of Inventory", "OVERALL INVENTORY COVERAGE", "#2563eb", is_weekly=is_weekly, is_percentage=False, fill=True), width="stretch")

    row2_left, row2_right = st.columns(2, gap="small")
    with row2_left:
        with st.container(border=True):
            _ = st.plotly_chart(create_styled_line_chart(kpi_data, "per_branch", "Per Branch OOS", "STOCKOUT RATE", "#0ea5e9", is_weekly=is_weekly, is_percentage=True), width="stretch")

    with row2_right:
        with st.container(border=True):
            _ = st.plotly_chart(create_styled_line_chart(kpi_data, "class_a_out", "Overall Class A Rate", "CLASS A STOCKOUT RATE", "#f43f5e", is_weekly=is_weekly, is_percentage=True), width="stretch")

    row3_left, row3_right = st.columns(2, gap="small")
    with row3_left:
        with st.container(border=True):
            _ = st.plotly_chart(create_styled_line_chart(kpi_data, "before_po", "Overall Before PO Balance", "STOCKOUT RATE BEFORE PO BALANCE", "#f59e0b", is_weekly=is_weekly, is_percentage=True), width="stretch")

    with row3_right:
        with st.container(border=True):
            _ = st.plotly_chart(create_styled_line_chart(kpi_data, "after_po", "Overall After PO Balance", "STOCKOUT RATE AFTER PO BALANCE", "#10b981", is_weekly=is_weekly, is_percentage=True, fill=True), width="stretch")
    ...
    with scope_col1:
        areas = ["All Areas"] + sorted([area for area in raw_data["area"].dropna().unique() if str(area).strip()])
        selected_area = st.selectbox("NETWORK SCOPE", areas, key="selected_scm_area_for_export")

    area_data = raw_data.copy()
    if selected_area != "All Areas": area_data = area_data[area_data["area"] == selected_area]
    ...
    section_heading("Performance Overview", f"Current raw-data stockout profile • {selected_area}")

    m1, m2, m3, m4 = st.columns(4, gap="small")
    m1.markdown(card_html("CLASS A RATE", rate_a, "HIGHEST-PRIORITY PARETO INVENTORY", "red", "A"), unsafe_allow_html=True)
    m2.markdown(card_html("CLASS B RATE", rate_b, "MEDIUM-PRIORITY PARETO INVENTORY", "yellow", "B"), unsafe_allow_html=True)
    m3.markdown(card_html("CLASS C RATE", rate_c, "LOWER-PRIORITY PARETO INVENTORY", "green", "C"), unsafe_allow_html=True)
    m4.markdown(card_html("AVERAGE RATE", avg_rate, "PERFORMANCE INDEX", "blue", "Σ"), unsafe_allow_html=True)

    # =========================================================
    # 9. STOCK OUT RATE PER AREA
    # =========================================================
    st.markdown("<br>", unsafe_allow_html=True)
    section_heading("Stock Out Rate per Area", "Average = mean of Class A/B/C rates • Class A = Class A Stock Out Count ÷ Class A Total Stock Status Count")
    ...
    for area, a_df in raw_data.groupby("area", sort=True, dropna=True):
        ...
        class_a_status = class_a_df["stock_status"].fillna("").astype(str).str.strip().str.casefold()
        class_a_stockout_count = int(class_a_status.eq("stockout").sum())
        class_a_total_stock_status_count = int(len(class_a_df))
        class_a_area_rate = round_half_up((class_a_stockout_count / class_a_total_stock_status_count) * 100) if class_a_total_stock_status_count > 0 else 0
        area_rates.append({"Area": area, "Average Stock Out Rate": avg_area_rate, "Class A Stock Out Rate": class_a_area_rate, "Class A Stock Out Count": class_a_stockout_count, "Class A Total Stock Status Count": class_a_total_stock_status_count})

    area_rates_df = pd.DataFrame(area_rates)
    if area_rates_df.empty:
        st.info("No area-level stockout data available.")
    else:
        area_rates_df = area_rates_df.sort_values("Average Stock Out Rate", ascending=False).reset_index(drop=True)
        area_order = area_rates_df["Area"].tolist()
        avg_col, class_a_col = st.columns(2, gap="small")

        with avg_col:
            fig_bar = px.bar(area_rates_df, x="Area", y="Average Stock Out Rate", text="Average Stock Out Rate", template=PLOTLY_TEMPLATE, title="Average Stock Out Rate per Area")
            _ = fig_bar.update_traces(texttemplate="<b>%{text:.0f}%</b>", textposition="outside", hovertemplate="<b>%{x}</b><br>Average Stock Out Rate: <b>%{y:.0f}%</b><extra></extra>")
            avg_bar_max = float(area_rates_df["Average Stock Out Rate"].max())
            _ = apply_executive_bar_style(fig_bar, accent_color="#6366f1", value_axis_title="Stockout rate", value_max=max(10, avg_bar_max * 1.24), category_order=area_order, orientation="v", percent=True, height=425)
            _ = fig_bar.update_layout(title_text="Average Stock Out Rate per Area<br><span style='font-size:10px;color:#94a3b8'>NETWORK RISK COMPARISON</span>")
            _ = st.plotly_chart(fig_bar, width="stretch", config=PLOTLY_BAR_CONFIG)

        with class_a_col:
            fig_class_a = px.bar(area_rates_df, x="Area", y="Class A Stock Out Rate", text="Class A Stock Out Rate", template=PLOTLY_TEMPLATE, title="Class A Stock Out Rate per Area", custom_data=["Class A Stock Out Count", "Class A Total Stock Status Count"])
            _ = fig_class_a.update_traces(texttemplate="<b>%{text:.0f}%</b>", textposition="outside", hovertemplate="<b>%{x}</b><br>Class A Stock Out Rate: <b>%{y:.0f}%</b><br>Class A Stock Out Count: <b>%{customdata[0]}</b><br>Class A Total Stock Status Count: <b>%{customdata[1]}</b><extra></extra>")
            class_a_bar_max = float(area_rates_df["Class A Stock Out Rate"].max())
            _ = apply_executive_bar_style(fig_class_a, accent_color="#f43f5e", value_axis_title="Class A stockout rate", value_max=max(10, class_a_bar_max * 1.24), category_order=area_order, orientation="v", percent=True, height=425)
            _ = fig_class_a.update_layout(title_text="Class A Stock Out Rate per Area<br><span style='font-size:10px;color:#94a3b8'>HIGHEST-PRIORITY PARETO RISK</span>")
    ...
    branch_rank_source["_normalized_class"] = branch_rank_source["pareto_class"].fillna("").astype(str).str.strip().str.casefold()
    branch_rank_source["_normalized_status"] = branch_rank_source["stock_status"].fillna("").astype(str).str.strip().str.casefold()
    branch_rank_source["branch"] = branch_rank_source["branch"].fillna("").astype(str).str.strip()
    branch_rank_source["area"] = branch_rank_source["area"].fillna("").astype(str).str.strip()

    branch_class_a = branch_rank_source[branch_rank_source["_normalized_class"].eq("class a") & branch_rank_source["branch"].ne("")].copy()
    if branch_class_a.empty:
        st.info("No Class A branch records are available for the selected network scope.")
    else:
        branch_class_a["_is_stockout"] = branch_class_a["_normalized_status"].eq("stockout").astype(int)
        branch_class_a_summary = branch_class_a.groupby(["area", "branch"], as_index=False, dropna=False).agg(**{"Class A Stock Out Count": ("_is_stockout", "sum"), "Class A Total Stock Status Count": ("_is_stockout", "size")})
        branch_class_a_summary["Class A Stock Out Rate"] = branch_class_a_summary.apply(lambda row: round_half_up((row["Class A Stock Out Count"] / row["Class A Total Stock Status Count"]) * 100) if row["Class A Total Stock Status Count"] > 0 else 0, axis=1)

        if selected_area == "All Areas":
            branch_class_a_summary["Branch Display"] = branch_class_a_summary["branch"] + "  •  " + branch_class_a_summary["area"]
        else:
            branch_class_a_summary["Branch Display"] = branch_class_a_summary["branch"]

        high_class_a_branches = branch_class_a_summary[branch_class_a_summary["Class A Stock Out Rate"] > 0].sort_values(["Class A Stock Out Rate", "Class A Stock Out Count", "Class A Total Stock Status Count", "branch"], ascending=[False, False, False, True]).head(TOP_BRANCH_LIMIT).reset_index(drop=True)
        zero_class_a_branches = branch_class_a_summary[branch_class_a_summary["Class A Stock Out Rate"] == 0].sort_values(["Class A Total Stock Status Count", "branch"], ascending=[False, True]).head(TOP_BRANCH_LIMIT).reset_index(drop=True)
        ...
        with high_rank_col:
            if high_class_a_branches.empty:
                ...
                high_rate_max = float(high_class_a_branches["Class A Stock Out Rate"].max())
                fig_high_class_a = px.bar(high_class_a_branches, x="Class A Stock Out Rate", y="Branch Display", orientation="h", text="Class A Stock Out Rate", template=PLOTLY_TEMPLATE, title=f"Top {len(high_class_a_branches)} Highest Class A Stock Out Rate", custom_data=["area", "branch", "Class A Stock Out Count", "Class A Total Stock Status Count"])
                _ = fig_high_class_a.update_traces(texttemplate="<b>%{text:.0f}%</b>", textposition="outside", hovertemplate="<b>%{customdata[1]}</b><br>Area: <b>%{customdata[0]}</b><br>Class A Stock Out Rate: <b>%{x:.0f}%</b><br>Class A Stock Out Count: <b>%{customdata[2]}</b><br>Class A Total Stock Status Count: <b>%{customdata[3]}</b><extra></extra>")
                _ = apply_executive_bar_style(fig_high_class_a, accent_color="#f43f5e", value_axis_title="Class A stockout rate", value_max=min(105, max(10, high_rate_max * 1.18)), category_order=high_order, orientation="h", percent=True, height=max(410, 44 * len(high_class_a_branches) + 130), right_margin=54)
                _ = fig_high_class_a.update_layout(title_text=f"Top {len(high_class_a_branches)} Highest Class A Stock Out Rate<br><span style='font-size:10px;color:#94a3b8'>PRIORITY BRANCH RISK RANKING</span>")
        ...
        with zero_rank_col:
            if zero_class_a_branches.empty:
                ...
                zero_coverage_max = float(zero_class_a_branches["Class A Total Stock Status Count"].max())
                fig_zero_class_a = px.bar(zero_class_a_branches, x="Class A Total Stock Status Count", y="Branch Display", orientation="h", text="Zero Rate Label", template=PLOTLY_TEMPLATE, title=f"Top {len(zero_class_a_branches)} Branches with 0% Class A Stock Out Rate", custom_data=["area", "branch", "Class A Stock Out Rate", "Class A Stock Out Count", "Class A Total Stock Status Count"])
                _ = fig_zero_class_a.update_traces(texttemplate="<b>%{text}</b>", textposition="outside", hovertemplate="<b>%{customdata[1]}</b><br>Area: <b>%{customdata[0]}</b><br>Class A Stock Out Rate: <b>%{customdata[2]:.0f}%</b><br>Class A Stock Out Count: <b>%{customdata[3]}</b><br>Class A Total Stock Status Count: <b>%{customdata[4]}</b><extra></extra>")
                _ = apply_executive_bar_style(fig_zero_class_a, accent_color="#10b981", value_axis_title="Class A stock-status coverage count", value_max=max(1, zero_coverage_max * 1.22), category_order=zero_order, orientation="h", percent=False, height=max(410, 44 * len(zero_class_a_branches) + 130), right_margin=66)
                _ = fig_zero_class_a.update_layout(title_text=f"Top {len(zero_class_a_branches)} Branches with 0% Class A Stock Out Rate<br><span style='font-size:10px;color:#94a3b8'>ZERO-OOS LEADERS BY CLASS A COVERAGE</span>")
    ...
    with head_col1:
        st.markdown("<div style='padding-top:12px; color:#94a3b8; font-size:0.82rem;'>Select a branch to review stockout risk and Pareto action models.</div>", unsafe_allow_html=True)
    with head_col2:
        branch_options = ["All Branches"] + sorted([branch for branch in area_data["branch"].dropna().unique() if str(branch).strip()])
        selected_branch = st.selectbox("BRANCH", branch_options)

    branch_data = area_data.copy()
    if selected_branch != "All Branches": branch_data = branch_data[branch_data["branch"] == selected_branch]
    ...
    c1.markdown(card_html("BRANCH CLASS A RATE", br_rate_a, "HIGH PRIORITY RISK", "red", "A"), unsafe_allow_html=True)
    c2.markdown(card_html("BRANCH CLASS B RATE", br_rate_b, "MEDIUM PRIORITY RISK", "yellow", "B"), unsafe_allow_html=True)
    c3.markdown(card_html("BRANCH CLASS C RATE", br_rate_c, "LOW PRIORITY RISK", "green", "C"), unsafe_allow_html=True)
    c4.markdown(card_html("BRANCH AVERAGE", br_avg, "PERFORMANCE INDEX", "blue", "Σ"), unsafe_allow_html=True)

    st.markdown("<br><br>", unsafe_allow_html=True)
    section_heading("Pareto Action Models", f"Transfer priorities for {selected_branch}")
    ...
    def render_pareto_table(df, pareto_class, hex_color):
        ...
        st.markdown(
            ...
            <div class='pareto-header' style='border-color: {hex_color};'>
                <span style='font-size:1.05rem; font-weight:900; color:{hex_color};'>{pareto_class.upper()}</span>
                <span class='pareto-count' style='color:{hex_color};'>● {item_count} Items</span>
            </div>
            """, unsafe_allow_html=True,
        )
        ...
        class_df["remaining_inventory"] = round_series_half_up(class_df["remaining_inventory"]).astype(int)
        class_df["suggested_transfer"] = round_series_half_up(class_df["suggested_transfer"]).astype(int)
        class_df["doi"] = round_series_half_up(class_df["doi"]).astype(int)
        class_df = class_df.sort_values(by=["suggested_transfer", "doi"], ascending=[False, True]).reset_index(drop=True)
        class_df.index += 1
        class_df = class_df.reset_index().rename(columns={"index": "Rank"})

        display_cols = ["Rank", "model", "stock_status", "remaining_inventory", "suggested_transfer", "doi"]
        ...
        table_html = (
            "<div class='pareto-html-shell'><table class='pareto-html-table'>"
            "<colgroup><col style='width:7%'><col style='width:35%'><col style='width:16%'><col style='width:14%'><col style='width:14%'><col style='width:14%'></colgroup>"
            "<thead><tr><th class='center'>Rank</th><th>Model</th><th class='center'>Status</th><th class='center'>Inventory</th><th class='center'>Transfer</th><th class='center'>DOI</th></tr></thead>"
            "<tbody>" + "".join(rows_html) + "</tbody></table></div>"
        )
        st.markdown(table_html, unsafe_allow_html=True)

    for pareto_class, pareto_color in [("Class A", "#f87171"), ("Class B", "#fbbf24"), ("Class C", "#4ade80")]:
        with st.container(border=True):
...
with tab_branch_requests:
    ...
    if not branch_options:
        ...
        branch_col, branch_info_col = st.columns([2.5, 5.5], gap="small")
        with branch_col:
        ...
        branch_area_values = [v for v in branch_source.get("area", pd.Series(dtype=str)).fillna("").astype(str).str.strip().unique().tolist() if v]
        branch_area = branch_area_values[0] if branch_area_values else "—"
        unique_model_count = int(branch_source["model"].nunique())

        with branch_info_col:
            st.markdown(
                f"""
                <div class='metric-card-base' style='min-height:88px;padding:14px 18px;'>
                    <div class='metric-header'>REQUESTING BRANCH <div class='icon-box'>B</div></div>
                    <div class='metric-value' style='font-size:1.35rem;margin:7px 0 4px 0;'>{html.escape(request_branch)}</div>
                    <div class='metric-footnote'>Area: {html.escape(branch_area)} • {unique_model_count:,} model(s) available in Raw_Data</div>
                </div>
                """,
                unsafe_allow_html=True,
            )

        # One model record per selected branch. If Raw_Data contains duplicate Branch + Model rows,
        # retain the first record to match the validation function's exact-match behavior.
        branch_models = (
            branch_source.sort_values(["model"]).drop_duplicates(subset=["branch", "model"], keep="first").copy()
        )
        editor_df = pd.DataFrame({
            "Model": branch_models["model"].astype(str).values,
            "Request Quantity": np.zeros(len(branch_models), dtype=int),
            "Remarks / Justification": [""] * len(branch_models),
            "Inventory": pd.to_numeric(branch_models["remaining_inventory"], errors="coerce").fillna(0).astype(int).values,
            "Stock Status": branch_models["stock_status"].fillna("").astype(str).values,
            "Current DoI": pd.to_numeric(branch_models["doi"], errors="coerce").fillna(0).round(2).values,
        })
        ...
        edited_requests = st.data_editor(
            ...
            hide_index=True,
            width="stretch",
            height=min(520, max(220, 38 * (len(editor_df) + 1))),
            key=f"branch_request_editor_{normalize_request_key(request_branch).replace(' ', '_')}",
            disabled=["Model", "Inventory", "Stock Status", "Current DoI"],
            column_config={
                ...
                "Remarks / Justification": st.column_config.TextColumn(
                    ...
                    width="large",
                    help="State why the branch needs the requested quantity.",
                ),
                "Inventory": st.column_config.NumberColumn("Current Inventory", format="%d"),
        ...
        if not active_input.empty:
            request_lines = [
                {
                    ...
                    "Request Quantity": int(max(safe_request_number(row["Request Quantity"], 0), 0)),
                    "Remarks / Justification": str(row["Remarks / Justification"] or "").strip(),
                }
                for _, row in active_input.iterrows()
            ]
            request_report = build_branch_request_report(request_source, request_lines)
            request_report = request_report[request_report["Branch"].astype(str).str.strip() == request_branch].copy()
            ...
            # Portrait-style on-screen presentation: each model is a vertical detail card rather than a wide row.
            for item_no, (_, row) in enumerate(request_report.reset_index(drop=True).iterrows(), start=1):
                ...
                new_doi_text = f"{float(row['New DoI']):.2f}" if pd.notna(row.get("New DoI")) else "N/A"
                current_doi_text = f"{float(row['Current DoI']):.2f}" if pd.notna(row.get("Current DoI")) else "N/A"
                new_inventory_text = f"{int(row['New Inventory']):,}" if pd.notna(row.get("New Inventory")) else "N/A"
                inventory_text = f"{int(row['Inventory']):,}" if pd.notna(row.get("Inventory")) else "N/A"
                remarks_text = str(row.get("Remarks / Justification", "") or "").strip() or "—"

                with st.container(border=True):
                    st.markdown(
                        f"<div style='font-weight:900;font-size:1.02rem;margin-bottom:0.55rem;'>REQUEST {item_no:02d} • {html.escape(str(row['Model']))}</div>",
                        unsafe_allow_html=True,
                    )
                    r1, r2, r3 = st.columns(3, gap="small")
                    r1.markdown(request_metric_card_html("REQUEST QTY", f"{int(row['Request Quantity']):,}", "UNITS REQUESTED", "blue", "Q"), unsafe_allow_html=True)
                    r2.markdown(request_metric_card_html("CURRENT INVENTORY", inventory_text, "RAW_DATA", "blue", "I"), unsafe_allow_html=True)
                    r3.markdown(request_metric_card_html("STOCK STATUS", status_text, "CURRENT STATUS", status_tone, "S"), unsafe_allow_html=True)
                    r4, r5, r6 = st.columns(3, gap="small")
                    r4.markdown(request_metric_card_html("CURRENT DOI", current_doi_text, "DAYS", "blue", "D"), unsafe_allow_html=True)
                    r5.markdown(request_metric_card_html("NEW INVENTORY", new_inventory_text, "AFTER REQUEST", "green", "+"), unsafe_allow_html=True)
                    r6.markdown(request_metric_card_html("NEW DOI", new_doi_text, "PROJECTED DAYS", "green", "N"), unsafe_allow_html=True)
                    st.markdown(
                        f"""
                        <div class='import-dialog-note' style='margin:0.45rem 0 0 0;background:rgba(245,158,11,0.06);'>
                            <b>Remarks / Justification:</b> {html.escape(remarks_text)}
                        </div>
                        """,
                        unsafe_allow_html=True,
                    )

            # Automatic report generation: bytes are regenerated every Streamlit rerun after any user edit.
            try:
                branch_request_xlsx = build_branch_request_excel(request_report, requesting_branch=request_branch)
                safe_branch_name = "".join(ch if ch.isalnum() or ch in {"-", "_"} else "_" for ch in request_branch).strip("_") or "Branch"
                download_col, note_col = st.columns([1.5, 4.5], gap="small")
                with download_col:
                    st.download_button(
                        "⬇ Generate / Download Portrait Report",
                        data=branch_request_xlsx,
                        file_name=f"Branch_Request_{safe_branch_name}_{date.today().strftime('%Y%m%d')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True,
                        key=f"branch_request_download_portrait_{normalize_request_key(request_branch).replace(' ', '_')}",
                    )
                with note_col:
                    st.caption(
            ...
            with st.expander("View Full Calculation Detail"):
